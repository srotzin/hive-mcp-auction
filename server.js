...
const _bogoDB = new _BogoDatabase(process.env.BOGO_DB_PATH || '/tmp/bogo_auction.db');
...
app.post('/v1/auction/open', async (req, res) => {
  // ── BOGO + 402 gate (runs before ENABLE_AUCTION check to avoid 503) ──
  const PRICE = 0.01;
  ...
  if (bogo.free) {
    ...
    return res.json({
      ...
      note: 'bogo applied — auction_open comped for this DID',
      queued: !ENABLE_AUCTION,
    });
  }
  ...
  if (!tx_hash) {
    return res.status(402).json({
      ...
      x402: {
        type: 'x402', version: '1', kind: 'auction_open',
        asking_usd: 0.01, accept_min_usd: 0.01,
      ...
      note: `Submit tx_hash in body or 'x402-tx-hash' header. Asking 0.01 USDC on Base to 0x15184bf50b3d3f52b60434f8942b7d52f2eb436e.`,
      did: did || null,
    });
  }
  ...
  // If service is disabled, return a paid-queued receipt rather than 503
  if (!ENABLE_AUCTION) {
    return res.json({
      ok: true, billed_usd: v.amount_usd, tx_hash,
      queued: true,
      note: 'Payment accepted. Auction service is warming up (ENABLE_AUCTION not yet set). Your slot is reserved.',
    });
  }

  // ── Original HMAC-authenticated auction logic ──────────────────────────
